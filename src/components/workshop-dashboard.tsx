...
const dynamicQuestionText: Record<Language, { title: string; description: string; choose: string; apply: string }> = {
  en: { title: "Questions for the initial problem", description: "Equipment-specific questions, anchored to your first reported symptom.", choose: "Choose", apply: "Use these answers" },
  fr: { title: "Questions liées au premier problème", description: "Questions adaptées à l’équipement et au premier symptôme décrit.", choose: "Choisir", apply: "Utiliser ces réponses" },
  bm: { title: "Ɲininkaliw gɛlɛnko fɔlɔ kan", description: "Ɲininkaliw bɛ ɲɛsin équipement ani gɛlɛnko fɔlɔ ma.", choose: "Sugandi", apply: "Nin jaabiw kɛ baara la" },
  zh: { title: "围绕首个问题的提问", description: "与设备和首个症状相关的问题。", choose: "请选择", apply: "使用这些答案" },
};
...
function EquipmentGuidedFlow({ language, onApply }: { language: Language; onApply: (summary: string) => void }) {
  const [equipment, setEquipment] = useState("motorcycle");
  const [fault, setFault] = useState("");
  const [detail, setDetail] = useState("");
  const text = dynamicQuestionText[language];
  const equipmentOptions: Array<[string, string]> = [["motorcycle", "Moto"], ["car", "Voiture"], ["phone", "Téléphone"], ["printer", "Imprimante"], ["solar", "Solaire"], ["ac", "Climatisation"], ["generator", "Groupe électrogène"], ["refrigerator", "Réfrigérateur"], ["computer", "Ordinateur"], ["other", "Autre équipement"]];
  const faults: Record<string, string[]> = { motorcycle: ["Ne démarre pas", "Cale", "Bruit moteur", "Freinage", "Électricité"], car: ["Ne démarre pas", "Cale", "Voyant", "Bruit", "Freinage"], phone: ["Ne s’allume pas", "Écran", "Batterie", "Charge", "Réseau"], printer: ["Bourrage papier", "N’imprime pas", "Qualité d’impression", "Erreur", "Bruit"], solar: ["Pas de courant", "Batterie", "Onduleur", "Panneau", "Câblage"], ac: ["Ne refroidit pas", "Fuite", "Bruit", "Ne démarre pas", "Télécommande"], generator: ["Ne démarre pas", "Pas de courant", "Surchauffe", "Bruit", "Carburant"], refrigerator: ["Ne refroidit pas", "Bruit", "Fuite", "Givre", "Ne démarre pas"], computer: ["Ne démarre pas", "Écran", "Lent", "Batterie", "Réseau"], other: ["Ne démarre pas", "Bruit", "Erreur visible", "Alimentation", "Autre"] };
  const details: Record<string, string[]> = { "Ne démarre pas": ["Aucun son", "Clic", "Voyant allumé", "Après une chute"], "Cale": ["Au ralenti", "À chaud", "À l’accélération", "Après quelques minutes"], "Bruit": ["Clic", "Claquement", "Sifflement", "Grincement"], "Bruit moteur": ["Claquement", "Sifflement", "Vibration", "À chaud"], "Pas de courant": ["Aucun voyant", "Batterie faible", "Après coupure", "Intermittent"], "Autre": ["Décrire dans le message", "Avec photo", "Avec son", "Je ne sais pas"] };
  const faultOptions = faults[equipment] ?? faults.other;
  const detailOptions = details[fault] ?? ["À froid", "À chaud", "Après utilisation", "Je ne sais pas"];
  return <Card className="border-zinc-200 bg-white"><CardHeader className="pb-3"><CardDescription>{text.title}</CardDescription><CardTitle className="mt-1 text-lg">Choisis l’équipement, puis la panne : les choix suivants s’adaptent.</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-3"><label className="text-sm text-zinc-700">Équipement<select value={equipment} onChange={(event) => { setEquipment(event.target.value); setFault(""); setDetail(""); }} className="mt-1 w-full rounded-md border border-zinc-300 bg-white p-2">{equipmentOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm text-zinc-700">Panne<select value={fault} onChange={(event) => { setFault(event.target.value); setDetail(""); }} className="mt-1 w-full rounded-md border border-zinc-300 bg-white p-2"><option value="">{text.choose}</option>{faultOptions.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-sm text-zinc-700">Détail<select value={detail} onChange={(event) => setDetail(event.target.value)} disabled={!fault} className="mt-1 w-full rounded-md border border-zinc-300 bg-white p-2"><option value="">{text.choose}</option>{detailOptions.map((item) => <option key={item}>{item}</option>)}</select></label><div className="sm:col-span-3"><Button type="button" size="sm" disabled={!fault} onClick={() => onApply(`Equipment: ${equipmentOptions.find(([value]) => value === equipment)?.[1]}. Fault: ${fault}. Detail: ${detail || "not specified"}. This is the initial guided context; continue from this fault only.`)}>{text.apply}</Button></div></CardContent></Card>;
}
    ...
    <main className="min-h-screen bg-stone-50 text-zinc-950">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1.65fr)_20rem] lg:px-8">
        <section className="space-y-5">
          ...
          {session ? <>{decision?.followUpQuestions?.length ? <DynamicQuestions key={decision.assistantMessage} decision={decision} language={language} onApply={setMessage} /> : <EquipmentGuidedFlow language={language} onApply={setMessage} />}</> : null}
