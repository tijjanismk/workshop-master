...
Return followUpQuestions with zero to three questions only when their answers would change the next diagnostic decision. Each question needs 2–5 concise, mutually exclusive choices in the user's language. Always anchor them to the first reported fault and adapt them to the detected equipment type, manufacturer, model, and current evidence; never return generic intake questions. Prefer the smallest useful set and return [] when the next safe action is already clear.
